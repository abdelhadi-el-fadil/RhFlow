...
def test_nominal_create_update_and_delete_by_admin(
    ...
    deleted = client.delete(
        f"/fiches-de-poste/{fiche['id']}", headers=_auth(admin_token)
    )
...
def test_directeur_can_create_and_update_only_in_his_direction(
    ...
    directeur = make_user(
        "directeur2@fiche.test", "Secret123!", role=UserRole.DIRECTEUR
    )
    other_directeur = make_user(
        "directeur3@fiche.test", "Secret123!", role=UserRole.DIRECTEUR
    )
...
def test_only_admin_and_drh_can_delete_fiche(
    ...
    directeur = make_user(
        "directeur4@fiche.test", "Secret123!", role=UserRole.DIRECTEUR
    )
...
def test_direction_filter_and_missing_resources(
    ...
    directeur = make_user(
        "directeur5@fiche.test", "Secret123!", role=UserRole.DIRECTEUR
    )
